...
#define IP6_MH_BU_S_BIT		0x0100	/* AnhKhuong: S bit in doctor Oh paper */
...
#define IP6_MH_BU_S_BIT		0x0001  /* AnhKhuong: S bit in doctor Oh paper */
#endif
...
#define IP6_MH_BA_S_BIT		0x10    /* S bit in doctor Oh paper */
...
struct ip6_mh_rot {
	struct ip6_mh	ip6mh_hdr;
	uint16_t	ip6mhrot_seqno;		/* Sequence Number */
	uint16_t	ip6mhrot_flags;
	uint16_t	ip6mhrot_lifetime;
	/* Followed by optional Mobility Options */
}__attribute__ ((packed));

struct ip6_mh_roi {
	struct ip6_mh	ip6mh_hdr;
	uint16_t	ip6mhroi_seqno;		/* Sequence Number */
	uint16_t	ip6mhroi_flags;
	uint16_t	ip6mhroi_lifetime;
	/* Followed by optional Mobility Options */
}__attribute__ ((packed));

struct ip6_mh_ros {
	struct ip6_mh	ip6mh_hdr;
	uint16_t	ip6mhros_seqno;		/* Sequence Number */
	uint16_t	ip6mhros_flags;
	uint16_t	ip6mhros_lifetime;
	/* Followed by optional Mobility Options */
}__attribute__ ((packed));

struct ip6_mh_rosa {
	struct ip6_mh	ip6mh_hdr;
	uint16_t	ip6mhrosa_seqno;		/* Sequence Number */
	uint16_t	ip6mhrosa_flags;
	uint16_t	ip6mhrosa_lifetime;
	/* Followed by optional Mobility Options */
}__attribute__ ((packed));

struct ip6_mh_etm {
	struct ip6_mh	ip6mh_hdr;
	uint16_t	ip6mhetm_reserve;
	/* Followed by optional Mobility Options */
}__attribute__ ((packed));

struct ip6_mh_hi {
	struct ip6_mh	ip6mh_hdr;
	uint16_t	ip6mhhi_reserve;
	/* Followed by optional Mobility Options */
}__attribute__ ((packed));

struct ip6_mh_ha {
	struct ip6_mh	ip6mh_hdr;
	uint16_t 	ip6mhha_status;	/* Status code */
	/* Followed by optional Mobility Options */
} __attribute__ ((packed));
...
#define IP6_MH_TYPE_ROT		8	/* RO Trigger Message */
#define IP6_MH_TYPE_ROI		9	/* RO Init Message */
#define IP6_MH_TYPE_ROS		10	/* RO Setup Message */
#define IP6_MH_TYPE_ROSA	11	/* RO Setup Ack Message */
#define	IP6_MH_TYPE_HI		12	/* Handover Initialize, for FPMIPv6 */
#define IP6_MH_TYPE_HA		13  /* Handover Initialize Ack, for FMIPv6 */
#define IP6_MH_TYPE_ETM	    14
...
#define IP6_MH_BAS_PRFX_DISCOV		1	/* Accepted, but prefix discovery
						   required */
#define IP6_MH_BAS_IHR_MSG			127 /* IHR message is PBA with status code = 127*/

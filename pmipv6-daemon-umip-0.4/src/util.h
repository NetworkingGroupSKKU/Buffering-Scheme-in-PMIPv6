...
// for timeval: tv_sec is second, usec is micro second = 1million second
#define ts2usec(tv) \
	((tv).tv_sec * TIME_SEC_USEC + (tv).tv_usec)

#define ts2dsec(tv) \
	((double)(tv).tv_sec + (double)(tv).tv_usec / TIME_SEC_USEC)

#define ts2msec(tv) \
	((tv).tv_sec * TIME_SEC_MSEC + (tv).tv_usec / TIME_SEC_MSEC)

//for timespec tv_sec is second, tv_nsec is nano second = 1billion second
#define tstomsec(tv) \
 ...
 *  Anh Khuong add
 */
static inline const char* getcurrenttime(char *str)
{
	struct timeval tv;
	struct timezone tz;
	struct tm *tm;
	...
	sprintf(str, "%d:%02d:%02d.%d", tm->tm_hour, tm->tm_min,

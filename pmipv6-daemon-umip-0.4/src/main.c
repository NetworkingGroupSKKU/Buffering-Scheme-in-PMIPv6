...
struct hotime_info time_info;
struct mip6_config conf;
struct mags_info mags_info;
int undefined_RO = 0;
#ifdef NAMYEONG_PAPER
mag_timeout_t mag_timeout;
#endif
...
int main(int argc, char **argv)
{
	...
	/* anhkhuong adds */
	memset(&time_info, 0, sizeof(struct hotime_info));
	memset(&mags_info, 0, sizeof(struct mags_info));
	mags_info.size = 2;
	inet_pton(AF_INET6, "2001:100::2", &mags_info.mags_addr[0]);
	inet_pton(AF_INET6, "2001:100::3", &mags_info.mags_addr[1]);
#ifdef NAMYEONG_PAPER
	memset(&mag_timeout, 0, sizeof(mag_timeout_t));
#endif

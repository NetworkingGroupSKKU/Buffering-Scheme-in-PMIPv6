...
struct hotime_info {
	...
	unsigned long period[20];		/* period for handover by micro second */
	int size;
};

struct mags_info {
	struct in6_addr mags_addr[5];
	int has_mn[5];					/* boolean to indentify MAG had MN info yet */
	int nhas_mn;					/* number mags had MN info */
	int size;
};

#ifdef NAMYEONG_PAPER
	typedef struct mag_timeout {
		int ttl_max_op;
		int ttl_max_np;
		int ttl_max_rs;
		int t_one_hop;
...
extern struct hotime_info time_info;
extern struct mags_info mags_info;
#ifdef NAMYEONG_PAPER
extern mag_timeout_t mag_timeout;
#endif
extern int undefined_RO;
